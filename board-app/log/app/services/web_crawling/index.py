...
def tag(board_id: str, site: str):
    ...
    try:
        ...
        logger.debug(
            f"DB 조회 결과 - TAG_Object_id: {TAG_Object_id}, TAG_object: {TAG_object}"
        )
        ...
        chatGPT = Tagsplit()
        logger.info(
            f"GPT 요약 요청 URL: https://gall.dcinside.com/board/view/?id=dcbest&no="
            + board_id)
        ...
        if TAG_object:
            ...
            logger.info(
                f"문서 업데이트 완료 - TAG_Object_id: {TAG_Object_id}, site: {site}, board_id: {board_id}"
            )
        else:
            logger.error(
                f"문서 조회 실패 - TAG_Object_id: {TAG_Object_id}, site: {site}, board_id: {board_id}"
            )
            return JSONResponse(
...
def board_summary(board_id: str, site: str):
    ...
    try:
        ...
        logger.debug(
            f"DB 조회 결과 - GPT_Object_id: {GPT_Object_id}, GPT_object: {GPT_object}"
        )
        ...
        if GPT_object:
            ...
            logger.info(
                f"문서 업데이트 완료 - GPT_Object_id: {GPT_Object_id}, site: {site}, board_id: {board_id}"
            )
        else:
            logger.error(
                f"문서 조회 실패 - GPT_Object_id: {GPT_Object_id}, site: {site}, board_id: {board_id}"
            )
            return JSONResponse(
                content={"detail": "해당 ObjectId로 문서를 찾을 수 없습니다."},
                status_code=404,
...
def get_real_time_best():
    ...
    for crawler in CrawllerList:
        ...
        try:
            logger.info(
                f"Starting real-time best fetch from {crawler.__class__.__name__}"
            )
            crawler.get_real_time_best()
            logger.info(
                f"Successfully fetched real-time best from {crawler.__class__.__name__}"
            )
        except Exception as e:
            logger.error(
                f"Error fetching real-time best from {crawler.__class__.__name__}: {str(e)}",
                exc_info=True,
            )
...
def get_daily_best():
    ...
    for crawler in CrawllerList:
        ...
        try:
            logger.info(
                f"Starting daily best fetch from {crawler.__class__.__name__}")
            crawler.get_daily_best()
            logger.info(
                f"Successfully fetched daily best from {crawler.__class__.__name__}"
            )
        except Exception as e:
            logger.error(
                f"Error fetching daily best from {crawler.__class__.__name__}: {str(e)}",
                exc_info=True,
            )
