...
| 메서드·내부 경로 | 설명 |
| --- | --- |
| `POST /api/boards/recommendations` | `tag`, `sites`, `category` 필터에 맞는 추천 ID와 이유 최대 120개 |
| `POST /api/boards/recommendations/posts` | `{ "ids": [...] }` 순서대로 게시글 최대 30개 조회 |
| `GET /api/boards/recommendations/profile` | 인증된 계정의 관심사 조회 |
| `POST /api/boards/recommendations/profile` | `{ "mutations": [...] }`로 행동·설정·초기화·비회원 기록 가져오기 반영 |

비회원은 추천 요청의 `profile`로 관심사를 전달하며 서버에 저장하지 않습니다.
회원은 인증된 공급자와 사용자 ID를 기준으로 서버 프로필을 사용하며 요청 본문의 프로필로 덮어쓰지 않습니다.
행동은 최근 30일/500건이고, 14일 반감기와 태그별 가중치를 적용합니다. 새 프로필 테이블과 정리 시각 인덱스는 기존 `Base.metadata.create_all` 흐름으로 생성됩니다.
만료된 행동은 즉시 추천에서 제외되고, 매시간 실행되는 정리 작업이 비활성 계정에서도 만료 데이터를 삭제합니다.
추천용 관심사 저장은 기존 읽은 글 표시/계정 기록 기능과 별개입니다.
