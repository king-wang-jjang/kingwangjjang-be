...
class AuthServiceTests(unittest.TestCase):
    ...
    def test_refresh_token_uses_browser_maximum_400_day_lifetime(self):
        ...
        self.assertEqual(decoded["user_id"], "123")
        self.assertEqual(int(decoded["expires_at"].timestamp()), payload["exp"])

    def test_refresh_tokens_are_hashed_with_sha256(self):
        token = JWTService.create_refresh_token("123", "kakao")

        digest = JWTService.hash_refresh_token(token)

        self.assertEqual(len(digest), 64)
        self.assertNotIn(token, digest)

    def test_refresh_token_without_expiry_is_rejected(self):
        token = jwt.encode(
            {
                "user_id": "123",
                "auth_provider": "kakao",
        ...
        self.assertIsNone(JWTService.decode_refresh_token(token))

    def test_rotate_session_rejects_a_token_that_is_not_current(self):
        token = JWTService.create_refresh_token("123", "kakao")

        class FakeRepository:
            def rotate_refresh_session(self, *args, **kwargs):
                return False
    ...
    def test_rotate_session_atomically_replaces_the_stored_hash_and_slides_expiry(self):
        token = JWTService.create_refresh_token("123", "kakao")
        ...
        self.assertEqual(repository.rotated[0:2], ("123", "kakao"))
        self.assertEqual(repository.rotated[2], JWTService.hash_refresh_token(token))
        self.assertEqual(repository.rotated[3], JWTService.hash_refresh_token(next_refresh_token))
        self.assertEqual(repository.rotated[4], JWTService.decode_refresh_token(next_refresh_token)["expires_at"])
        self.assertEqual(repository.rotated[5], token)
    ...
    def test_save_user_to_db_creates_a_hashed_independent_session(self):
        class FakeRepository:
            def __init__(self):
                self.saved = None
        ...
        refresh_token = JWTService.create_refresh_token("123", "kakao")
        with patch("app.services.auth_service.UserRepository", lambda: repository):
            saved = UserService.save_user_to_db(user_info, refresh_token)

        self.assertEqual(saved, {"user_id": "123"})
        self.assertEqual(
            repository.saved[0:5],
            (
                "123",
                "kakao",
                "왕짱",
        ...
        self.assertEqual(repository.saved[5], JWTService.decode_refresh_token(refresh_token)["expires_at"])

    def test_save_user_to_db_rejects_a_token_for_another_user(self):
        user_info = UserType(user_id="123", auth_provider="kakao")
        refresh_token = JWTService.create_refresh_token("456", "kakao")
